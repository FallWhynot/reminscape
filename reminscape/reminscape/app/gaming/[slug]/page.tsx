...
export async function generateStaticParams() {
  return getArticlesByCategory("Gaming").map((a) => ({ slug: a.slug }));
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getArticleBySlug(slug);
  if (!article || article.category !== "Gaming") notFound();
  const related = getArticlesByCategory("Gaming").filter((a) => a.slug !== slug).slice(0, 3);
