...
export async function generateStaticParams() {
  return getArticlesByCategory("News").map((a) => ({ slug: a.slug }));
}
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = getArticlesByCategory("News")
    .filter((a) => a.slug !== slug)
    .slice(0, 3);
  const allArticles = articles;
  const idx = allArticles.findIndex((a) => a.slug === slug);
  const prev = idx > 0 ? allArticles[idx - 1] : undefined;
  const next = idx < allArticles.length - 1 ? allArticles[idx + 1] : undefined;
